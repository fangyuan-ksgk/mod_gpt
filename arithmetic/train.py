...
def train(args):
    device = args.device
    print(f"{'═' * 60}")
    print(f"  ops={args.ops} | n_abs_tokens={args.n_abs_tokens} | device={device}")
    ...
    # Quirke: 20% linear warmup + 80% cosine decay
    warmup_steps = int(args.num_steps * 0.2)
        ...
        if step % args.log_every == 0:
            ...
            with torch.no_grad():
                eval_tokens, _, _ = generate_batch(
                    128, args.n_digits, ops=args.ops,
            ...
            print(f"step {step:6d} | loss {loss.item():.4f} | acc {acc:.3f} | {elapsed:.0f}s{marker}")
            if acc > best_acc:
                best_acc = acc
