...
# 3 batches × 4 experiments each = 12 experiments total
...
# Prevent OpenBLAS thread explosion when running warmup clustering
...
MAX_ITER=2
TEMP=1.0
NUM_ROLLOUTS=4  # used by warmup/v3 baseline, v5 ignores it
...
#   All use warmup_sft for fair comparison
# ============================================================
echo ""
echo "==============================="
...
run_exp "b1_ste_traj1_abs05" \
    --use_v5 --warmup_sft --warmup_sft_steps 500 --warmup_emb_lr_mult 10.0 \
...
# ============================================================
# BATCH 3: Warmup + K variation
#   Does warmup help? How does K interact with STE?
# ============================================================
echo ""
echo "======================================"
echo "  BATCH 3: Warmup + K variation"
echo "======================================"

# Exp 3a: No warmup (random init abstractions + STE)
run_exp "b3_ste_no_warmup" \
    --use_v5 \
    --alpha_traj 1.0 --alpha_abs 0.5 --alpha_contrastive 1.0 --gamma_contrastive 0.5 \
    --corrupt_method shuffle --corrupt_ratio 0.3

# Exp 3b: K=2 (more frequent abstractions)
run_exp "b3_ste_K2" \
    --use_v5 --warmup_sft --warmup_sft_steps 500 --warmup_emb_lr_mult 10.0 \
    --alpha_traj 1.0 --alpha_abs 0.5 --alpha_contrastive 1.0 --gamma_contrastive 0.5 \
    --corrupt_method shuffle --corrupt_ratio 0.3 \
...
# Exp 3c: K=8 (sparser abstractions)
run_exp "b3_ste_K8" \
    --use_v5 --warmup_sft --warmup_sft_steps 500 --warmup_emb_lr_mult 10.0 \
    --alpha_traj 1.0 --alpha_abs 0.5 --alpha_contrastive 1.0 --gamma_contrastive 0.5 \
    --corrupt_method shuffle --corrupt_ratio 0.3 \
...
# Exp 3d: v5 STE full recipe (best guess: warmup + hinge + anchor + zipf)
run_exp "b3_ste_full" \
    --use_v5 --warmup_sft --warmup_sft_steps 500 --warmup_emb_lr_mult 10.0 \
    --alpha_traj 1.0 --alpha_abs 0.5 --alpha_contrastive 1.0 --gamma_contrastive 0.5 \
    --corrupt_method shuffle --corrupt_ratio 0.3 \
