...
for step in range(train_steps + 1):
    ...
    if last_step or (args.val_loss_every > 0 and step % args.val_loss_every == 0):
        ...
        log_str = f"step:{step}/{train_steps} "
        for dname, metrics in results.items():
            ppl = metrics['entropy']
            log_str += f"[{dname} ppl: {ppl:.4f}] "
            loss_record[f"val_{dname}_entropy"].append(ppl)
            # You can also log MBE if you want
            
        print0(f"{log_str} train_time:{training_time_ms:.0f}ms", console=True)
