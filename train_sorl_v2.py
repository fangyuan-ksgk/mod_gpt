# SoRL on GAT (pre-training) || SGPO + EMA model (attempts to unify exploration & exploitation -- offline distillation)
# ----------------------------------------------------------------------------------------------------------------------
...
for i in range(warmup_steps):
    ...
    with torch.no_grad():
        search_tokens, search_ppt, search_adv = sorl_search(tokens, ref_model, n=args.num_rollouts, K=args.K, max_iterations=args.max_iterations, memory_span=memory_span, attn_blocksize=attn_blocksize, 
...
for step in range(train_steps + 1):
    ...
    for accum_step in range(train_accumulation_steps): 
        ...
        with torch.no_grad(): 
            search_tokens, search_ppt, search_adv = sorl_search(tokens, ref_model, n=args.num_rollouts, K=args.K, max_iterations=args.max_iterations, 
