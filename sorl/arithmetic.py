...
def comp_acc(model, val_loader, tokenizer, K, max_iterations, memory_span, attn_blocksize, temperatures_eval):
    ...
    for i in tqdm(range(50)):
        ...
        question = tokenizer.decode(idx[0].tolist()[1:-1])
        for i in range(len(answer_idx[0])*2): 
            ...
            monologue = idx[idx >= model.vocab_sizes[0]]

        is_correct, pred_answer, true_answer = check_answer(idx_without_abstraction, answer_idx, tokenizer)
        # print(f"question: {question} | is_correct: {is_correct} | pred_answer: {pred_answer} | true_answer: {true_answer} | monologue: {monologue.tolist()}")
        record.append({"question": question, "pred": pred_answer, "answer": true_answer, "monologue": monologue.tolist()})
