...
for ABSTRACT_VOCAB_SIZE in 16; do
  for K in 8 4 32 64; do
    torchrun \
      ...
      --alpha_info_gain $ALPHA_INFO_GAIN \
      --avoid_prefix_truncation \
      --run_info "TinyStories no prefix truncation, with attention sweep, K=$K, abs_vocab=$ABSTRACT_VOCAB_SIZE)"
  done
done
...
for ABSTRACT_VOCAB_SIZE in 16; do
  for K in 8 4 32 64; do
    torchrun \
      ...
      --alpha_info_gain $ALPHA_INFO_GAIN \
      --no_attn_sweep \
      --run_info "TinyStories with prefix truncation, K=$K, abs_vocab=$ABSTRACT_VOCAB_SIZE)"
  done
done
